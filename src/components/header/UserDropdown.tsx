...
export default function UserDropdown() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        console.log(res)
        const data = res.data as { metadata: UserProfile };
  ...
  if (!profile) return <div>Không tìm thấy thông tin cửa hàng</div>;
  ...
  const handleLogout = async () => {
    const res = await axiosClientFe.get(`/api/auth`, {
      headers: {
        'x-client-id': shopId,
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="block mr-1 font-medium text-theme-sm">{profile?.usr_first_name || "pls, update name"} {profile?.usr_last_name}</span>
